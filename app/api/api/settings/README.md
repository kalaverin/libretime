...
- the `prod.py` (`api.settings.prod`) module is the django settings entrypoint. The module contains bindings between the user configuration and the django settings. **Advanced users** may edit this file to better integrate the LibreTime API in their setup.
- the `testing.py` (`api.settings.testing`) module is the testing django settings entrypoint.
