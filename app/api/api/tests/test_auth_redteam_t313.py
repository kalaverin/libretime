...
class TestUserTokenBOLA:
    ...
    def test_access_other_user_token(self, api_client, admin_user, regular_user):
        """Try to access another user's token."""
        token = baker.make("core.UserToken", user=admin_user, token="admin_secret_token")

        api_client.force_authenticate(user=regular_user)
        response = api_client.get("/api/v2/user-tokens/admin_secret_token")

        if response.status_code == 200:
            pytest.fail("CRITICAL BAG: Can access other user's token (BOLA)")

    def test_delete_other_user_token(self, api_client, admin_user, regular_user):
        ...
        if response.status_code == 204:
            pytest.fail("CRITICAL BAG: Can delete other user's token (BOLA)")

    def test_list_shows_only_own_tokens(self, api_client, admin_user, regular_user):
        """Verify list returns only user's own tokens."""
        admin_token = baker.make("core.UserToken", user=admin_user, token="admin_token")
        user_token = baker.make("core.UserToken", user=regular_user, token="user_token")
        ...
        if "admin_token" in tokens:
            pytest.fail("CRITICAL BAG: List shows other users' tokens (BOLA)")
...
class TestUserTokenMassAssignment:
    ...
    def test_create_token_for_other_user(self, api_client, admin_user, regular_user):
        ...
        if response.status_code == 201:
            ...
            if data.get("user") == admin_user.id:
                pytest.fail("CRITICAL BAG: Can create token for other user")

    def test_update_token_value(self, api_client, admin_user):
        """Try to change token value via PATCH."""
        token = baker.make("core.UserToken", user=admin_user, token="original_token")
        ...
        if response.status_code == 200:
            pytest.fail("BAG: Can modify token value (should be immutable)")
...
class TestLoginAttemptBOLA:
    ...
    def test_access_other_user_login_attempt(self, api_client, admin_user, regular_user):
        """Try to access another user's login attempt record."""
        attempt = baker.make("core.LoginAttempt", ip="192.168.1.100", attempts=5)
    ...
    def test_modify_login_attempt_count(self, api_client, admin_user):
        ...
        if response.status_code == 200:
            pytest.fail("CRITICAL BAG: Can modify login attempt counter")
    ...
    def test_delete_login_attempt_record(self, api_client, admin_user):
        ...
        if response.status_code == 204:
            pytest.fail("BAG: Can delete login attempt record (evasion)")
...
class TestLoginAttemptInjection:
    ...
    def test_create_login_attempt_with_xff_header(self, api_client, admin_user):
...
class TestAuthBusinessLogic:
    ...
    def test_create_token_without_auth(self, api_client):
        ...
        if response.status_code == 201:
            pytest.fail("CRITICAL BAG: Anonymous can create token")
    ...
    def test_list_tokens_without_auth(self, api_client):
        ...
        if response.status_code == 200:
            pytest.fail("CRITICAL BAG: Anonymous can list tokens")
