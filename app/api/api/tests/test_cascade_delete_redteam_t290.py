...
from api.schedule.models import Show, ShowInstance, Playlist, PlaylistContent, SmartBlock, SmartBlockContent
...
class TestCascadeDeleteBOLA:
    ...
    @pytest.mark.xfail(reason="T863: BOLA - attacker can delete victim's show (flaky: passes alone, fails in suite)")
    def test_bola_delete_show_cascades_to_instances(self, api_client, faker):
        ...
        assert response.status_code in [403, 404], f"T863: BOLA - attacker deleted show (got {response.status_code})"

    @pytest.mark.django_db(transaction=True)
    @pytest.mark.xfail(reason="T864: BOLA - attacker can delete victim's playlist (flaky: state-dependent)")
    def test_bola_delete_playlist_cascades_to_contents(self, api_client, faker):
        ...
        contents = [baker.make(PlaylistContent, playlist=playlist, position=i) for i in range(10)]
        ...
        assert response.status_code in [403, 404], f"T864: BOLA - attacker deleted playlist (got {response.status_code})"

    @pytest.mark.django_db(transaction=True)
    @pytest.mark.xfail(reason="T865: BOLA - attacker can delete victim's smartblock (state-dependent)")
    def test_bola_delete_smartblock_cascades_to_contents(self, api_client, faker):
        """Attacker deleting victim's smartblock cascades to contents."""
        victim = baker.make(User, username=f"victim_{faker.user_name()}")
        attacker = baker.make(User, username=f"attacker_{faker.user_name()}")
        
        # Victim's smartblock with contents
        block = baker.make(SmartBlock, name="Victim's SmartBlock", owner=victim, kind=SmartBlock.Kind.STATIC)
        contents = [baker.make(SmartBlockContent, block=block, position=i) for i in range(5)]
        content_ids = [c.id for c in contents]
        ...
        assert response.status_code in [403, 404], f"T865: BOLA - attacker deleted smartblock (got {response.status_code})"

    @pytest.mark.django_db(transaction=True)
    @pytest.mark.xfail(reason="T866: BOLA - attacker can delete victim's library (flaky: state-dependent)")
    def test_bola_delete_library_with_files(self, api_client, faker):
        """Attacker deleting victim's library is blocked."""
        victim = baker.make(User, username=f"victim_{faker.user_name()}")
        attacker = baker.make(User, username=f"attacker_{faker.user_name()}")
        
        library = baker.make(Library, code="VICTIMLIB", name="Victim's Library", description="Test")
        files = [baker.make(File, name=f"file{i}.mp3", mime="audio/mp3", library=library, owner=victim, size=1024, accessed=0) for i in range(5)]
        ...
        assert response.status_code in [403, 404, 400, 409], f"T866: BOLA/500 - library deletion (got {response.status_code})"
...
class TestCascadeDeleteDoS:
    ...
    def test_mass_cascade_delete_show_instances(self, api_client, admin_user, faker):
        ...
        assert duration < 5, f"T867: Cascade delete too slow: {duration}s for 100 instances"

    @pytest.mark.django_db
    def test_mass_cascade_delete_playlist_contents(self, api_client, admin_user, faker):
        """Deleting playlist with thousands of contents."""
        playlist = baker.make(Playlist, name="Playlist with Many Contents", owner=admin_user)
        
        # Create many contents
        contents = [baker.make(PlaylistContent, playlist=playlist, position=i) for i in range(100)]
        ...
        assert duration < 5, f"T868: Cascade delete too slow: {duration}s for 100 contents"

    @pytest.mark.django_db
    @pytest.mark.xfail(reason="T869: No rate limiting on cascade delete")
    def test_rapid_cascade_delete_requests(self, api_client, admin_user, faker):
...
class TestCascadeDeleteRaceCondition:
    ...
    @pytest.mark.django_db
    @pytest.mark.xfail(reason="T870: Race condition - concurrent delete of same show")
    def test_race_condition_concurrent_show_delete(self, api_client, admin_user, faker):
        ...
        assert success_count == 1, f"T870: Race condition - {success_count} deletes succeeded instead of 1"

    @pytest.mark.django_db
    def test_race_condition_delete_while_adding_content_safe(self, api_client, admin_user, faker):
...
class TestCascadeDeleteFKConstraintBypass:
    ...
    @pytest.mark.django_db
    @pytest.mark.xfail(reason="T872: FK constraint bypass - nullify FK before delete")
    def test_fk_bypass_nullify_before_delete(self, api_client, admin_user, faker):
        """Try to nullify FK references before cascade delete."""
        library = baker.make(Library, code="TESTLIB", name="Test Library", description="Test")
        file_obj = baker.make(
            File,
            name="test.mp3",
            mime="audio/mp3",
        ...
        if response.status_code == 200:
            ...
            if data.get("library") is None:
                ...
                if del_response.status_code == 204:
                    pytest.fail("T872: FK constraint bypassed - nullified FK then deleted parent")

    @pytest.mark.django_db
    @pytest.mark.xfail(reason="T873: FK constraint violation causes 500 error")
    def test_delete_with_active_references_blocked(self, api_client, admin_user, faker):
        """Delete parent with active child references should fail gracefully."""
        library = baker.make(Library, code="REFLIB", name="Ref Library", description="Test")
        file_obj = baker.make(
            File,
            name="ref.mp3",
            mime="audio/mp3",
        ...
        assert response.status_code in [400, 409], f"T873: FK constraint caused {response.status_code} instead of graceful error"
...
class TestCascadeDeleteInjection:
    ...
    @pytest.mark.django_db
    def test_sqli_in_delete_cascade_trigger(self, api_client, admin_user, faker):
        ...
        if response.status_code >= 500:
            pytest.fail(f"T874: SQLi in name caused 500 during cascade delete")
...
class TestCascadeDeleteDataLeakage:
    ...
    @pytest.mark.django_db
    def test_error_message_leaks_child_count(self, api_client, admin_user, faker):
...
class TestCascadeDeleteOrphanedData:
    ...
    @pytest.mark.django_db
    def test_interrupted_cascade_no_orphans(self, api_client, admin_user, faker):
        ...
        for iid in instance_ids:
            if ShowInstance.objects.filter(id=iid).exists():
                pytest.fail(f"T876: Orphaned instance {iid} after cascade delete")

    @pytest.mark.django_db
    def test_cascade_vs_manual_delete_consistency(self, api_client, admin_user, faker):
...
class TestCascadeDeleteAPIPermissions:
    ...
    def test_delete_without_auth(self, api_client, faker):
        ...
        assert response.status_code in [401, 403], f"T877: Delete without auth succeeded: {response.status_code}"
    ...
    def test_delete_with_invalid_token(self, api_client, faker):
        ...
        assert response.status_code in [401, 403], f"T878: Delete with invalid token succeeded: {response.status_code}"

    @pytest.mark.django_db
    def test_cascade_deletes_children_with_parent_permission(self, api_client, admin_user, faker):
        """Cascade delete might bypass permission checks on children."""
        # Create playlist where user has delete permission
        playlist = baker.make(Playlist, name="Parent Playlist", owner=admin_user)
        
        # Create contents that user might not have direct permission to delete
        contents = [baker.make(PlaylistContent, playlist=playlist, position=i) for i in range(3)]
        content_ids = [c.id for c in contents]
