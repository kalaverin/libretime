...
class TestSmartBlockContentIDOR:
    ...
    def test_list_content_shows_only_own_blocks(self, api_client, admin_user, regular_user):
        """Verify user can only see content from their own blocks."""
        from model_bakery import baker
        
        # Admin's block with content
        admin_block = baker.make("schedule.SmartBlock", name="Admin Block", owner=admin_user)
        ...
        user_block = baker.make("schedule.SmartBlock", name="User Block", owner=regular_user)
    ...
    def test_access_other_user_content(self, api_client, admin_user, regular_user):
        ...
        api_client.force_authenticate(user=regular_user)
        response = api_client.get(f"/api/v2/smart-block-contents/{admin_content.id}")
        
        if response.status_code == 200:
            pytest.xfail("BOLA: User can access other user's smart block content")

    def test_create_content_in_other_user_block(self, api_client, admin_user, regular_user):
...
class TestSmartBlockContentInvalidReferences:
    ...
    def test_create_with_other_user_file(self, api_client, admin_user, regular_user):
...
class TestSmartBlockContentDelete:
    ...
    def test_delete_other_user_content(self, api_client, admin_user, regular_user):
        ...
        api_client.force_authenticate(user=regular_user)
        response = api_client.delete(f"/api/v2/smart-block-contents/{admin_content.id}")
    ...
    def test_delete_without_auth(self, api_client, admin_user):
        ...
        response = api_client.delete(f"/api/v2/smart-block-contents/{content.id}")
        
        if response.status_code != 403:
            pytest.xfail(f"SECURITY: Anonymous delete returned {response.status_code}")