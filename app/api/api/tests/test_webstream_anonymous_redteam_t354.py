...
class TestWebstreamAnonymousCreate:
    ...
    def test_anonymous_create_webstream_blocked(self, anonymous_client):
        ...
        response = anonymous_client.post("/api/v2/webstreams", data, format="json")
...
class TestWebstreamAnonymousRetrieve:
    ...
    def test_anonymous_list_webstreams_blocked(self, anonymous_client, admin_user):
        """Anonymous cannot list webstreams."""
        baker.make(Webstream, name="Test Stream", url="http://test.com/stream", owner=admin_user)
    ...
    def test_anonymous_retrieve_single_blocked(self, anonymous_client, admin_user):
        """Anonymous cannot retrieve individual webstream."""
        stream = baker.make(Webstream, name="Test Stream", url="http://test.com/stream", owner=admin_user)
...
class TestWebstreamAnonymousUpdate:
    ...
    def test_anonymous_patch_blocked(self, anonymous_client, admin_user):
        ...
        stream = baker.make(Webstream, name="Original", url="http://test.com/stream", owner=admin_user)
    ...
    def test_anonymous_patch_created_at_blocked(self, anonymous_client, admin_user):
        """Anonymous cannot manipulate created_at via PATCH."""
        stream = baker.make(Webstream, name="Test", url="http://test.com/stream", owner=admin_user)
        ...
        assert stream.created_at == original_created, "T354: created_at was modified by anonymous"
...
class TestWebstreamAnonymousDelete:
    ...
    def test_anonymous_delete_blocked(self, anonymous_client, admin_user):
        ...
        stream = baker.make(Webstream, name="To Delete", url="http://test.com/stream", owner=admin_user)
        stream_id = stream.id
        ...
        assert Webstream.objects.filter(id=stream_id).exists(), "T354: Stream was deleted by anonymous"
...
class TestWebstreamCreatedAtImmutability:
    ...
    @pytest.mark.xfail(reason="T354: created_at is mutable - needs serializer fix")
    def test_authenticated_cannot_modify_created_at(self, api_client, admin_user):
        ...
        if response.status_code == 200:
            ...
            assert stream.created_at == original_created, "T354: created_at was modified"
        else:
