...
class TestSmartBlockContentInformationDisclosure:
    ...
    def test_error_message_on_invalid_filter(self, api_client, admin_user):
        ...
        response = api_client.get(
            "/api/v2/smart-block-contents?block=invalid"
        )
...
class TestSmartBlockContentIDORWithFilter:
    ...
    def test_filter_shows_only_own_content(self, api_client, admin_user, regular_user):
    ...
    def test_filter_by_other_user_block(self, api_client, admin_user, regular_user):
        ...
        content_ids = [c["id"] for c in data]
        if admin_content.id in content_ids:
            pytest.xfail("BOLA: Can filter by other user's block and see content")
...
class TestSmartBlockContentMassOrdering:
    ...
    def test_extract_all_content_via_ordering(self, api_client, admin_user, regular_user):
