        ...
        def create_pref():
            try:
                response = client.post(
                    "/api/v2/preferences",
                    {"key": "race_test_key", "value": "value", "user": admin_user.id},
                    format="json"
                )
...
class TestPreferenceSQLInjection:
    ...
    def test_sqli_in_key_field_union_select(self, api_client, admin_user):
    ...
    def test_sqli_in_value_field(self, api_client, admin_user):
        ...
        for payload in sqli_payloads:
            response = api_client.post(
                "/api/v2/preferences",
                {"key": f"val_test_{hash(payload)}", "value": payload, "user": admin_user.id},
                format="json"
            )
            # Should store value as-is without executing
            assert response.status_code in [201, 400]

    def test_sqli_time_based_in_key(self, api_client, admin_user):
        ...
        response = api_client.post(
            "/api/v2/preferences",
            {"key": "key'||pg_sleep(5)||'", "value": "test", "user": admin_user.id},
            format="json"
        )
...
class TestPreferenceUnicodeBypass:
    ...
    def test_unicode_normalized_same_key(self, api_client, admin_user):
    ...
    def test_null_byte_in_key(self, api_client, admin_user):
    ...
    def test_newline_in_key(self, api_client, admin_user):
...
class TestPreferenceMassAssignment:
    ...
    def test_create_with_id_field(self, api_client, admin_user):
        ...
        response = api_client.post(
            "/api/v2/preferences",
            {"id": 999999, "key": "id_test", "value": "value", "user": admin_user.id},
            format="json"
        )
    ...
    def test_create_with_invalid_user_id(self, api_client, admin_user):
    ...
    def test_create_for_other_user(self, api_client, admin_user, regular_user):
        """Try to create preference for another user (horizontal privilege escalation)."""
        api_client.force_authenticate(user=admin_user)
        
        response = api_client.post(
            "/api/v2/preferences",
            {"key": "other_user_key", "value": "value", "user": regular_user.id},
            format="json"
        )
...
class TestPreferenceInformationDisclosure:
    ...
    def test_integrity_error_message_leakage(self, api_client, admin_user):
    ...
    def test_error_message_uniqueness(self, api_client, admin_user):
...
class TestPreferencePartialIndexBypass:
    ...
    def test_site_pref_then_user_pref_same_key(self, api_client, admin_user):
        ...
        response2 = api_client.post(
            "/api/v2/preferences",
            {"key": "shared_key", "value": "user_value", "user": admin_user.id},
            format="json"
        )
        assert response2.status_code == 201

    def test_user_pref_then_site_pref_same_key(self, api_client, admin_user):
        ...
        response1 = api_client.post(
            "/api/v2/preferences",
            {"key": "shared_key2", "value": "user_value", "user": admin_user.id},
            format="json"
        )
        assert response1.status_code == 201
    ...
    def test_two_site_prefs_same_key_fails(self, api_client, admin_user):
...
class TestPreferenceKeyLength:
    ...
    def test_empty_key(self, api_client, admin_user):
    ...
    def test_whitespace_only_key(self, api_client, admin_user):
