...
class TestPodcastIDOR:
    ...
    def test_list_podcasts_shows_only_own(self, api_client, admin_user, regular_user):
        ...
        if admin_podcast.id in podcast_ids:
            pytest.xfail("T353: BOLA vulnerability - user can see other users' podcasts")

    def test_access_other_user_podcast_directly(self, api_client, admin_user, regular_user):
        ...
        if response.status_code == 200:
            pytest.xfail("T353: BOLA vulnerability - user can access other user's podcast")

    def test_modify_other_user_podcast(self, api_client, admin_user, regular_user):
    ...
    def test_delete_other_user_podcast(self, api_client, admin_user, regular_user):
...
class TestPodcastOwnerMassAssignment:
    ...
    def test_create_podcast_with_other_user_as_owner(self, api_client, admin_user, regular_user):
    ...
    def test_change_podcast_owner_to_another_user(self, api_client, admin_user, regular_user):
    ...
    def test_create_podcast_with_invalid_owner_id(self, api_client, admin_user):
    ...
    def test_create_podcast_with_negative_owner_id(self, api_client, admin_user):
...
class TestPodcastEpisodeBOLA:
    ...
    def test_access_episode_of_other_user_podcast(self, api_client, admin_user, regular_user):
        ...
        api_client.force_authenticate(user=regular_user)
        response = api_client.get(f"/api/v2/podcast-episodes/{admin_episode.id}")
        
        # T353: Currently vulnerable - returns 200
        # Expected: 403 or 404 (denied)
        if response.status_code == 200:
            pytest.xfail("T353: BOLA vulnerability - user can access other user's episode")

    def test_modify_episode_of_other_user_podcast(self, api_client, admin_user, regular_user):
...
class TestPodcastPermissionsBypass:
    ...
    def test_admin_can_access_any_podcast(self, api_client, admin_user, regular_user):
    ...
    def test_anonymous_cannot_list_podcasts(self, api_client):
        ...
        # Expected: 403
        if response.status_code == 200:
            pytest.xfail("T353: Anonymous users can list podcasts - missing auth")
    ...
    def test_anonymous_cannot_create_podcast(self, api_client):
        ...
        # Expected: 403
        if response.status_code == 201:
            pytest.xfail("T353: Anonymous users can create podcasts - missing auth")
...
class TestPodcastStationIDOR:
    ...
    def test_list_station_podcasts_shows_only_own(self, api_client, admin_user, regular_user):
        ...
        admin_station = baker.make("podcasts.StationPodcast", podcast=admin_podcast)
        
        # User creates station podcast
        user_podcast = baker.make("podcasts.Podcast", owner=regular_user)
        user_station = baker.make("podcasts.StationPodcast", podcast=user_podcast)
...
class TestPodcastImportedIDOR:
    ...
    def test_access_other_user_imported_podcast(self, api_client, admin_user, regular_user):
        """Try to access another user's imported podcast."""
        from model_bakery import baker
        
        # Admin creates imported podcast
        admin_podcast = baker.make("podcasts.Podcast", owner=admin_user)
        admin_imported = baker.make("podcasts.ImportedPodcast", podcast=admin_podcast)
        
        # User tries to access
        api_client.force_authenticate(user=regular_user)
        response = api_client.get(f"/api/v2/imported-podcasts/{admin_imported.id}")
