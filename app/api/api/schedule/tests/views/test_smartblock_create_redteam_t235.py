...
class TestSmartBlockCreateRedTeam:
    ...
    @pytest.mark.xfail(reason="T427: Mass assignment - created_at manipulation")
    def test_bopla_mass_assignment_created_at(self, api_client):
        ...
        if response.status_code == 201:
            data = response.json()
            assert "2020" not in data.get("created_at", ""), \
...
class TestSmartBlockCreateAdvancedRedTeam:
    ...
    @pytest.mark.xfail(reason="T431: Mass assignment - updated_at manipulation")
    def test_bopla_mass_assignment_updated_at(self, api_client):
        ...
        if response.status_code == 201:
            data = response.json()
            assert "2030" not in data.get("updated_at", ""), \
                "BOPLA: updated_at was set to future"

    @pytest.mark.xfail(reason="T432: Mass assignment - length field manipulation")
    def test_bopla_mass_assignment_length(self, api_client):
        ...
        if response.status_code == 201:
            ...
            assert data.get("length") is None or response.status_code == 400, \
    ...
    @pytest.mark.xfail(reason="T433: Race condition - duplicate names possible")
