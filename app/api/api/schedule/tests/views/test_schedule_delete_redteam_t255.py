...
class TestScheduleDeleteRedTeam:
    ...
    @pytest.mark.xfail(reason="T599: BOLA - DELETE returns wrong status for other's schedule")
    def test_bola_delete_other_users_schedule_status(self, api_client, faker):
        ...
        if response.status_code == 204:
            pytest.fail("T599: BOLA - Successfully deleted other user's schedule!")
        elif response.status_code == 404:
            pytest.fail("T599: Info leak - 404 reveals schedule doesn't exist (should be 403)")

    def test_bola_batch_delete_scope(self, api_client, faker):
        ...
        api_client.delete(f"/api/v2/schedule/{schedules[0].id}")
        final_count = Schedule.objects.count()

        if initial_count - final_count > 1:
            pytest.fail(f"Batch delete affected {initial_count - final_count} schedules")
    ...
    @pytest.mark.xfail(reason="T600: Auth - Invalid token returns 200 instead of 403")
    def test_delete_with_invalid_token(self, api_client, faker):
        ...
        try:
            response = api_client.delete("/api/v2/schedule/1")
            if response.status_code == 200:
                pytest.fail("T600: Invalid token accepted - authentication bypass")
            assert response.status_code == 403
        finally:
    ...
    def test_race_condition_concurrent_delete(self, api_client, faker):
        ...
        def delete_schedule():
            return api_client.delete(f"/api/v2/schedule/{schedule.id}").status_code
        ...
        if success_count != 1:
            pytest.fail(f"Race condition - {success_count} deletes succeeded, expected 1")
        if not_found_count != 4:
            pytest.fail(f"Race condition - {not_found_count} got 404, expected 4")

    @pytest.mark.xfail(reason="T601: Mass deletion - No rate limiting on delete")
    def test_mass_deletion_rate_limit(self, api_client, faker):
    ...
    def test_http_method_override_delete(self, api_client, faker):
    ...
    def test_sqli_in_delete_id(self, api_client, faker):
        ...
        for payload in sqli_payloads:
            response = api_client.delete(f"/api/v2/schedule/{payload}")
            assert response.status_code in [400, 404], \
                f"SQLi '{payload}' caused {response.status_code}"

    def test_path_traversal_in_delete_id(self, api_client, faker):
        ...
        for test_id in traversal_ids:
            response = api_client.delete(f"/api/v2/schedule/{test_id}")
            assert response.status_code in [400, 404], \
    ...
    def test_unicode_in_delete_id(self, api_client, faker):
        """Validation: Unicode in DELETE id handled gracefully."""
        response = api_client.delete("/api/v2/schedule/日本語")
        assert response.status_code in [400, 404]

    def test_negative_id_delete(self, api_client, faker):
        """Validation: Negative ID in DELETE."""
        response = api_client.delete("/api/v2/schedule/-1")
        assert response.status_code == 404

    def test_null_bytes_in_delete_id(self, api_client, faker):
        """Validation: Null bytes in DELETE id."""
        response = api_client.delete("/api/v2/schedule/1%00test")
        assert response.status_code in [400, 404]
    ...
    @pytest.mark.xfail(reason="T602: Info Leak - DELETE error reveals schedule existence")
    def test_error_message_leaks_existence_delete(self, api_client, faker):
        ...
        response_existing = api_client.delete(f"/api/v2/schedule/{victim_schedule.id}")
        response_nonexistent = api_client.delete("/api/v2/schedule/999999")
    ...
    def test_id_enumeration_timing_attack(self, api_client, faker):
        ...
        start = time.time()
        api_client.delete(f"/api/v2/schedule/{schedule.id}")
        time_existing = time.time() - start

        start = time.time()
        api_client.delete("/api/v2/schedule/999999")
        time_nonexistent = time.time() - start

        if time_existing > 0:
            ratio = max(time_existing, time_nonexistent) / min(time_existing, time_nonexistent)
            if ratio > 3:
    ...
    def test_double_delete_returns_404(self, api_client, faker):
