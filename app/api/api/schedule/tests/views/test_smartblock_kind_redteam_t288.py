...
class TestSmartBlockKindBOLA:
    ...
    @pytest.mark.xfail(reason="T829: BOLA - no ownership check in SmartBlockViewSet")
    def test_bola_retrieve_other_users_smartblock(self, api_client, faker):
        ...
        assert response.status_code == 403, f"T829: BOLA - attacker accessed victim smart block (got {response.status_code})"
    ...
    def test_bola_list_shows_all_smartblocks(self, api_client, faker):
        ...
        assert "Victim Secret SmartBlock" not in block_names, "T830: BOLA - LIST shows victim's private smart blocks"

    @pytest.mark.django_db
    @pytest.mark.xfail(reason="T831: BOLA - attacker can update victim's smart block kind")
    def test_bola_update_other_users_smartblock_kind(self, api_client, faker):
        ...
        assert response.status_code in [403, 404], f"T831: BOLA - attacker updated victim's smart block (got {response.status_code})"

    @pytest.mark.django_db
    @pytest.mark.xfail(reason="T832: BOLA - attacker can delete victim's smart block")
    def test_bola_delete_other_users_smartblock(self, api_client, faker):
        ...
        assert response.status_code in [403, 404], f"T832: BOLA - attacker deleted victim's smart block (got {response.status_code})"
...
class TestSmartBlockKindBOPLA:
    ...
    def test_bopla_mass_assignment_id_field(self, api_client, admin_user, faker):
        ...
        if response.status_code == 201:
            data = response.json()
            assert data.get("id") != 999999, "T833: BOPLA - mass assignment of id field allowed"

    @pytest.mark.django_db
    @pytest.mark.xfail(reason="T834: BOPLA - mass assignment of created_at allowed")
    def test_bopla_mass_assignment_created_at(self, api_client, admin_user, faker):
        ...
        data = response.json()
        assert "2020" not in str(data.get("created_at", "")), "T834: BOPLA - mass assignment of created_at allowed"
    ...
    def test_bopla_change_owner_via_update(self, api_client, admin_user, regular_user, faker):
        ...
        assert response.status_code == 200
        data = response.json()
        assert data.get("owner") != regular_user.id, "T835: BOPLA - owner change via PATCH allowed"
    ...
    def test_bopla_extra_fields_not_rejected(self, api_client, admin_user, faker):
        ...
        assert response.status_code == 400, f"T836: BOPLA - extra fields silently accepted (got {response.status_code})"
...
class TestSmartBlockKindValidation:
    ...
    def test_invalid_kind_values_rejected(self, api_client, admin_user, faker):
        ...
        for kind in invalid_kinds:
            ...
            assert response.status_code == 400, f"T837: Invalid kind '{kind}' accepted"
    ...
    def test_kind_case_sensitivity(self, api_client, admin_user, faker):
        ...
        if response.status_code == 201:
            data = response.json()
            assert data.get("kind") == "static", "T838: Uppercase kind not normalized"
    ...
    def test_kind_with_whitespace(self, api_client, admin_user, faker):
        ...
        if response.status_code == 201:
            data = response.json()
            assert data.get("kind") == "static", "T839: Whitespace not trimmed from kind"
...
class TestSmartBlockKindInjection:
    ...
    def test_sqli_in_kind_field_create(self, api_client, admin_user):
        ...
        for payload in SQLI_PAYLOADS[:5]:
            ...
            sql_errors = ["sql", "sqlite", "mysql", "postgresql", "syntax error"]
    ...
    def test_nosql_injection_kind_field(self, api_client, admin_user):
        ...
        for payload in NOSQL_PAYLOADS:
            ...
            if response.status_code == 201:
                pytest.fail(f"T843: NoSQL object accepted in kind field: {payload}")
...
class TestSmartBlockKindLogic:
    ...
    @pytest.mark.django_db
    def test_change_kind_with_criteria_static_to_dynamic(self, api_client, admin_user, faker):
    ...
    @pytest.mark.django_db
    def test_change_kind_dynamic_to_static_with_criteria(self, api_client, admin_user, faker):
    ...
    @pytest.mark.django_db
    def test_create_dynamic_without_criteria(self, api_client, admin_user, faker):
...
class TestSmartBlockKindFuzzing:
    ...
    def test_fuzzing_kind_field_create(self, api_client, admin_user):
        ...
        for payload in INVALID_KIND_VALUES[:15]:
            ...
            if response.status_code >= 500:
                pytest.fail(f"T844: Fuzzing payload caused 500: {str(payload)[:50]}")
    ...
    def test_fuzzing_kind_filter(self, api_client, admin_user):
        ...
        for payload in INVALID_KIND_VALUES[:10]:
            ...
            if response.status_code >= 500:
                pytest.fail(f"T845: Kind filter fuzzing caused 500: {str(payload)[:50]}")
...
class TestSmartBlockKindDoS:
    ...
    def test_rapid_create_requests(self, api_client, admin_user):
        ...
        assert success_count < 50, "T847: No rate limiting on smart-block CREATE"
...
class TestSmartBlockKindFilter:
    ...
    def test_filter_by_kind_static(self, api_client, admin_user, faker):
        ...
        baker.make(SmartBlock, name="Static 1", owner=admin_user, kind=SmartBlock.Kind.STATIC)
        baker.make(SmartBlock, name="Dynamic 1", owner=admin_user, kind=SmartBlock.Kind.DYNAMIC)
    ...
    def test_filter_by_invalid_kind(self, api_client, admin_user, faker):
        ...
        if response.status_code == 200:
            data = response.json()
            assert len(data) == 0, "T848: Invalid kind filter returns non-empty results"
    ...
    def test_error_message_leaks_structure(self, api_client, admin_user):
        ...
        if response.status_code >= 400:
            ...
            for pattern in sensitive_patterns:
                assert pattern not in response_text, f"T849: Error message leaks DB structure: {pattern}"
