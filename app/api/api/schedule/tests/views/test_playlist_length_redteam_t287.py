...
class TestPlaylistLengthBOLA:
    ...
    @pytest.mark.xfail(reason="T806: BOLA - no ownership check in PlaylistViewSet")
    def test_bola_retrieve_other_users_playlist_length(self, api_client, faker):
        ...
        assert response.status_code == 403, f"T806: BOLA - attacker accessed victim playlist (got {response.status_code})"

    @pytest.mark.django_db
    @pytest.mark.xfail(reason="T807: BOLA - LIST shows all playlists, no owner filtering")
    def test_bola_list_shows_all_playlists(self, api_client, faker):
        ...
        assert "Victim Secret Playlist" not in playlist_names, "T807: BOLA - LIST shows victim's private playlists"

    @pytest.mark.django_db
    @pytest.mark.xfail(reason="T808: BOLA - attacker can update victim's playlist")
    def test_bola_update_other_users_playlist_length(self, api_client, faker):
        ...
        assert response.status_code in [403, 404], f"T808: BOLA - attacker updated victim's playlist (got {response.status_code})"

    @pytest.mark.django_db
    @pytest.mark.xfail(reason="T809: BOLA - attacker can delete victim's playlist")
    def test_bola_delete_other_users_playlist(self, api_client, faker):
        ...
        assert response.status_code in [403, 404], f"T809: BOLA - attacker deleted victim's playlist (got {response.status_code})"
...
class TestPlaylistLengthBOPLA:
    ...
    def test_bopla_mass_assignment_id_field(self, api_client, admin_user, faker):
        ...
        if response.status_code == 201:
            ...
            if data.get("id") == 999999:
                pytest.fail("T810: BOPLA - mass assignment of id field allowed")

    @pytest.mark.django_db
    @pytest.mark.xfail(reason="T811: BOPLA - mass assignment of created_at allowed")
    def test_bopla_mass_assignment_created_at(self, api_client, admin_user, faker):
        ...
        data = response.json()
        assert "2020" not in str(data.get("created_at", "")), "T811: BOPLA - mass assignment of created_at allowed"
    ...
    def test_bopla_change_owner_via_update(self, api_client, admin_user, regular_user, faker):
        ...
        assert response.status_code == 200
        data = response.json()
        assert data.get("owner") != regular_user.id, "T812: BOPLA - owner change via PATCH allowed"
    ...
    def test_bopla_extra_fields_not_rejected(self, api_client, admin_user, faker):
        ...
        assert response.status_code == 400, f"T813: BOPLA - extra fields silently accepted (got {response.status_code})"
...
class TestPlaylistLengthInjection:
    ...
    def test_sqli_in_length_field_create(self, api_client, admin_user):
        ...
        for payload in SQLI_PAYLOADS[:5]:
            ...
            if response.status_code >= 500:
                pytest.fail(f"T814: SQLi in length field causes 500: {payload}")
    ...
    def test_sqli_in_length_field_update(self, api_client, admin_user):
        ...
        for payload in SQLI_PAYLOADS[:5]:
            ...
            if response.status_code >= 500:
                pytest.fail(f"T815: SQLi in length UPDATE causes 500: {payload}")
    ...
    def test_nosql_injection_length_field(self, api_client, admin_user):
        ...
        for payload in nosql_payloads:
            ...
            if response.status_code == 201:
                pytest.fail(f"T816: NoSQL object accepted in length field: {payload}")
...
class TestPlaylistLengthValidation:
    ...
    def test_invalid_time_format_accepted(self, api_client, admin_user):
        ...
        for fmt in invalid_formats:
            ...
            assert response.status_code != 201, f"T817: Invalid time format accepted: {fmt}"

    @pytest.mark.django_db
    @pytest.mark.xfail(reason="T818: Overflow length value 999999:00:00 accepted")
    def test_overflow_length_value(self, api_client, admin_user):
        ...
        for val in overflow_values:
            ...
            assert response.status_code != 201, f"T818: Overflow length value accepted: {val}"
...
class TestPlaylistLengthFuzzing:
    ...
    def test_fuzzing_length_field(self, api_client, admin_user):
        ...
        for payload in NAUGHTY_STRINGS[:15]:
            ...
            # Should not crash
            if response.status_code >= 500:
                pytest.fail(f"T820: Fuzzing payload caused 500: {payload[:50]}")
...
class TestPlaylistLengthDoS:
    ...
    def test_rapid_create_requests(self, api_client, admin_user):
        ...
        assert success_count < 50, "T823: No rate limiting on playlist CREATE (50 requests succeeded)"
...
class TestPlaylistLengthEdgeCases:
    ...
    def test_max_valid_length(self, api_client, admin_user):
        ...
        if response.status_code not in [201, 400]:
            pytest.fail(f"T824: Unexpected status for max length: {response.status_code}")
...
class TestPlaylistLengthMethodBypass:
    ...
    def test_put_vs_patch_length_update(self, api_client, admin_user):
        ...
        if response.status_code == 200:
            ...
            if "description" not in data or data.get("description") != "Test description":
        ...
        if response.status_code >= 400:
            ...
            for pattern in sensitive_patterns:
                if pattern in response_text:
                    pytest.fail(f"T827: Error message leaks DB structure: {pattern}")
