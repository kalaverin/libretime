...
def validate_hex_color(value: Any, field_name: str = "color") -> str:
    ...
    if not _HEX_COLOR_PATTERN.match(value):
        raise ValidationError(
            _("{field} must be a valid 6-digit hex color (e.g., FF0000).").format(
                field=field_name
            ),
            code=f"{field_name}_invalid_format",
        )
...
def validate_max_length(value: Any, max_length: int, field_name: str = "value") -> Any:
...
def validate_url_length(value: Any, max_length: int = 2048, field_name: str = "url") -> Any:
    """Validate URL length."""
...
def validate_choice(value: Any, valid_choices: set[str], field_name: str = "value") -> Any:
