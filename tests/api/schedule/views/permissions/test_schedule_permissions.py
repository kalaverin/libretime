...
class TestScheduleViewSetPermissions:
    ...
    def setup(self, admin_client, admin_user, host_client):
    ...
    def test_list_allows_anonymous(self, admin_client):
    ...
    def test_admin_can_delete_schedule(self):
        ...
        response = self.admin_client.delete(f"/api/v2/schedule/{schedule.id}")
    ...
    def test_list_admin_sees_all_schedules(self):
        ...
        response = self.admin_client.get("/api/v2/schedule")
        assert response.status_code == 200
        data = response.json()
        assert len(data) == 2
