...
class TestShowViewSetDelete:
    ...
    def test_delete_show_success_returns_204(self, admin_client):
        """DELETE should return 204 on successful deletion."""
        show = baker.make(Show, name="Test Show")
        response = admin_client.delete(f"/api/v2/shows/{show.id}")
        assert response.status_code == 204

    def test_delete_show_removes_from_db(self, admin_client):
        """DELETE should remove show from database."""
        show = baker.make(Show, name="Test Show")
        admin_client.delete(f"/api/v2/shows/{show.id}")
        assert Show.objects.filter(id=show.id).count() == 0

    def test_delete_show_not_found_returns_404(self, admin_client):
        """DELETE non-existent show should return 404."""
        response = admin_client.delete("/api/v2/shows/999999")
    ...
    def test_delete_show_with_hosts(self, admin_client):
    ...
    def test_delete_show_with_days(self, admin_client):
    ...
    def test_delete_show_with_instances(self, admin_client):
    ...
    def test_delete_show_double_delete_returns_404(self, admin_client):
    ...
    def test_delete_show_wrong_method_returns_405(self, admin_client):
    ...
    def test_delete_show_returns_empty_body(self, admin_client):
        """DELETE should return empty response body."""
        show = baker.make(Show, name="Test Show")
        response = admin_client.delete(f"/api/v2/shows/{show.id}")
        assert response.content == b""

    def test_delete_show_id_zero_returns_404(self, admin_client):
        """DELETE with id=0 should return 404."""
        response = admin_client.delete("/api/v2/shows/0")
        assert response.status_code == 404

    def test_delete_show_negative_id_returns_404(self, admin_client):
        """DELETE with negative id should return 404."""
        response = admin_client.delete("/api/v2/shows/-1")
        assert response.status_code == 404

    def test_delete_show_large_id_returns_404(self, admin_client):
        """DELETE with very large id should return 404."""
        response = admin_client.delete("/api/v2/shows/999999999")
        assert response.status_code == 404

    def test_delete_show_sql_injection_attempt(self, admin_client):
        """DELETE with SQL injection in id should be handled safely."""
        response = admin_client.delete("/api/v2/shows/1 OR 1=1")
        assert response.status_code == 404

    def test_delete_show_path_traversal_attempt(self, admin_client):
        """DELETE with path traversal should be handled safely."""
        response = admin_client.delete("/api/v2/shows/../../../etc/passwd")
        assert response.status_code == 404

    def test_delete_show_special_chars_in_id(self, admin_client):
        """DELETE with special chars in id should return 404."""
        response = admin_client.delete("/api/v2/shows/test%20id")
        assert response.status_code == 404
