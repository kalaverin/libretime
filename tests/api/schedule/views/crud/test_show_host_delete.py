...
class TestShowHostViewSetDelete:
    ...
    def test_delete_show_host_success_returns_204(self, admin_client):
        ...
        response = admin_client.delete(f"/api/v2/show-hosts/{show_host.id}")
        assert response.status_code == 204

    def test_delete_show_host_removes_from_db(self, admin_client):
        """DELETE should remove show host from database."""
        show = baker.make(Show, name="Test Show")
        user = baker.make(User, username="testhost1")
        show_host = baker.make(ShowHost, show=show, user=user)

        admin_client.delete(f"/api/v2/show-hosts/{show_host.id}")
        assert ShowHost.objects.filter(id=show_host.id).count() == 0

    def test_delete_show_host_not_found_returns_404(self, admin_client):
        """DELETE non-existent show host should return 404."""
        response = admin_client.delete("/api/v2/show-hosts/999999")
    ...
    def test_delete_show_host_double_delete_returns_404(self, admin_client):
    ...
    def test_delete_show_host_returns_empty_body(self, admin_client):
    ...
    def test_delete_one_host_others_remain(self, admin_client):
        ...
        admin_client.delete(f"/api/v2/show-hosts/{show_host2.id}")
    ...
    def test_delete_host_from_one_show_keeps_other_shows(self, admin_client):
    ...
    def test_delete_show_host_id_zero_returns_404(self, admin_client):
        """DELETE with id=0 should return 404."""
        response = admin_client.delete("/api/v2/show-hosts/0")
        assert response.status_code == 404

    def test_delete_show_host_negative_id_returns_404(self, admin_client):
        """DELETE with negative id should return 404."""
        response = admin_client.delete("/api/v2/show-hosts/-1")
        assert response.status_code == 404

    def test_delete_show_host_sql_injection_attempt(self, admin_client):
        """DELETE with SQL injection in id should be handled safely."""
        response = admin_client.delete("/api/v2/show-hosts/1 OR 1=1")
        assert response.status_code == 404
