...
class TestSmartBlockDeleteRedTeam:
    ...
    def test_bola_delete_other_users_block(self, admin_client):
        ...
        response = admin_client.delete(f"/api/v2/smart-blocks/{victim_block.id}")
    ...
    def test_bola_cascade_delete_other_user_content(self, admin_client):
    ...
    def test_bola_delete_leaks_block_existence(self, admin_client):
        ...
        # Try to delete it
        response_real = admin_client.delete(
            f"/api/v2/smart-blocks/{secret_block.id}",
        )

        # Try to delete non-existent block
        response_fake = admin_client.delete("/api/v2/smart-blocks/999999")
    ...
    def test_idor_sql_injection_in_delete_path(self, admin_client):
        ...
        for bad_id in sqli_ids:
            response = admin_client.delete(f"/api/v2/smart-blocks/{bad_id}")
    ...
    def test_idor_path_traversal_delete(self, admin_client):
        ...
        for path in traversal_paths:
            response = admin_client.delete(f"/api/v2/smart-blocks/{path}")
            assert response.status_code in [
                404,
                400,
            ], f"Path traversal caused {response.status_code}"

    def test_idor_unicode_id_delete(self, admin_client):
        ...
        for uid in unicode_ids:
            response = admin_client.delete(f"/api/v2/smart-blocks/{uid}")
            assert response.status_code in [
                404,
                400,
            ], f"Unicode ID '{uid}' caused {response.status_code}"

    def test_idor_float_id_delete(self, admin_client):
        """IDOR: Float ID in DELETE."""
        response = admin_client.delete("/api/v2/smart-blocks/1.5")
        assert response.status_code in [
            404,
            400,
        ], f"Float ID accepted: {response.status_code}"

    def test_idor_scientific_notation_id(self, admin_client):
        """IDOR: Scientific notation in ID."""
        response = admin_client.delete("/api/v2/smart-blocks/1e5")
        assert response.status_code in [
            404,
            400,
    ...
    def test_race_double_delete_concurrent(self, admin_client):
    ...
    def test_race_delete_and_update_concurrent(self, admin_client):
        ...
        def delete_block():
            return admin_client.delete(f"/api/v2/smart-blocks/{block.id}")
    ...
    def test_http_method_override_on_delete(self, admin_client):
    ...
    def test_http_delete_with_body(self, admin_client):
    ...
    def test_cascade_delete_mass_content(self, admin_client):
    ...
    def test_cascade_delete_many_criteria(self, admin_client):
    ...
    def test_fuzzing_naughty_strings_in_delete_url(self, admin_client):
        ...
        for string in naughty_strings:
            response = admin_client.delete(f"/api/v2/smart-blocks/{string}")
            assert response.status_code in [
                404,
                400,
            ], f"Naughty string '{string}' caused {response.status_code}"

    def test_fuzzing_large_id_delete(self, admin_client):
        ...
        for lid in large_ids:
            response = admin_client.delete(f"/api/v2/smart-blocks/{lid}")
            assert (
    ...
    def test_timing_delete_existing_vs_nonexisting(self, admin_client):
        ...
        start = time.time()
        admin_client.delete(f"/api/v2/smart-blocks/{block.id}")
        time_existing = time.time() - start

        # Time delete of non-existing block
        start = time.time()
        admin_client.delete("/api/v2/smart-blocks/999999")
