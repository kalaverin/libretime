...
class TestPlaylistDeleteRedTeam:
    ...
    def test_bola_delete_other_users_playlist_fails(self, admin_client):
    ...
    @pytest.mark.xfail(reason="T420: BOLA vulnerability - no owner filtering")
    def test_bola_delete_multiple_other_users_playlists(self, admin_client):
        ...
        for pid in playlist_ids:
            admin_client.delete(f"/api/v2/playlists/{pid}")
    ...
    def test_bola_id_prediction_delete_sequential(self, admin_client):
        ...
        for offset in range(-5, 10):
            ...
            admin_client.delete(f"/api/v2/playlists/{predicted_id}")
    ...
    def test_bopla_mass_assignment_via_delete_response(self, admin_client):
    ...
    def test_sqli_delete_id_union_select(self, admin_client):
        ...
        for payload in sql_payloads:
            response = admin_client.delete(f"/api/v2/playlists/{payload}")
    ...
    def test_nosql_injection_delete(self, admin_client):
    ...
    def test_path_traversal_delete(self, admin_client):
    ...
    def test_delete_rate_limiting(self, admin_client):
    ...
    def test_delete_non_numeric_id_performance(self, admin_client):
        """Resource consumption: Complex ID should not cause DoS."""
        # Very long ID that might cause regex backtracking
        long_id = "A" * 10000

        start_time = time.time()
        response = admin_client.delete(f"/api/v2/playlists/{long_id}")
        elapsed = time.time() - start_time
    ...
    def test_delete_with_trace_method(self, admin_client):
        """Security misconfig: TRACE method should not delete."""
        user = baker.make(User, username="testred_user")
        playlist = baker.make(Playlist, name="Test", owner=user)

        # Try TRACE request (should not delete)
        response = admin_client.trace(f"/api/v2/playlists/{playlist.id}")
    ...
    def test_delete_after_ownership_transfer(self, admin_client):
    ...
    def test_double_delete_race_condition(self, admin_client):
        """Race condition: Simultaneous delete requests."""
        user = baker.make(User, username="testred_user")
        playlist = baker.make(Playlist, name="Test", owner=user)

        def delete_attempt():
            return admin_client.delete(f"/api/v2/playlists/{playlist.id}")
    ...
    def test_delete_unicode_id(self, admin_client):
        ...
        for uid in unicode_ids:
            response = admin_client.delete(f"/api/v2/playlists/{uid}")
            assert response.status_code in [
                400,
                404,
            ], f"Unicode ID '{repr(uid)}' caused {response.status_code}"

    def test_delete_boolean_id(self, admin_client):
        ...
        for bid in bool_ids:
            response = admin_client.delete(f"/api/v2/playlists/{bid}")
            assert response.status_code in [
                400,
                404,
            ], f"Boolean ID '{bid}' caused {response.status_code}"

    def test_delete_null_and_empty(self, admin_client):
        """Input validation: null and empty ID handling."""
        # Null ID (trailing slash)
        response = admin_client.delete("/api/v2/playlists/null")
        assert response.status_code == 404

        # Empty ID (double slash)
        response = admin_client.delete("/api/v2/playlists/")
    ...
    def test_delete_error_message_information_disclosure(self, admin_client):
        ...
        for tid in test_ids:
            response = admin_client.delete(f"/api/v2/playlists/{tid}")
    ...
    def test_delete_timing_attack(self, admin_client):
        ...
        start = time.time()
        admin_client.delete("/api/v2/playlists/999999")
