...
class TestManagerShowPermissions:
    ...
    def test_manager_can_update_any_show(self, manager_client, manager_user, faker):
        """MANAGER can UPDATE shows they host (change_show permission)."""
        show = baker.make(Show, name=f"Manager Show {faker.uuid4()[:8]}")
        from api.schedule.models import ShowHost
        ShowHost.objects.create(show=show, user=manager_user)
    ...
    def test_manager_can_delete_any_show(self, manager_client, manager_user, faker):
        """MANAGER can DELETE shows they host (delete_show permission)."""
        show = baker.make(Show, name=f"Manager Show {faker.uuid4()[:8]}")
        from api.schedule.models import ShowHost
        ShowHost.objects.create(show=show, user=manager_user)
        show_id = show.id
