...
from api.schedule.models import Playlist, Show, ShowHost, SmartBlock, Webstream
...
def create_entity(entity_type: str, owner: User | None, faker):
    ...
    if entity_type == "show":
        ...
        if owner is not None:
            ShowHost.objects.create(show=show, user=owner)
        return show
...
class TestDeleteManagerAnyObject:
    ...
    def test_manager_can_delete_show(self, manager_client, manager_user, faker):
        """MANAGER can delete shows they host."""
        show = create_entity("show", manager_user, faker)
        obj_id = show.id
