...
class TestFileViewSetDelete:
    ...
    def test_delete_file_not_allowed_returns_409(self, admin_client):
    ...
    def test_delete_file_removes_from_db(self, admin_client):
    ...
    def test_delete_file_not_found(self, admin_client):
    ...
    def test_delete_file_scheduled_future_returns_409(self, admin_client):
    ...
    def test_delete_file_not_allowed_no_filepath(self, admin_client):
    ...
    def test_delete_file_not_allowed_not_on_disk(self, admin_client):
    ...
    def test_delete_file_removes_from_db(self, admin_client):
        ...
        with patch("api.storage.views.file.os.path.isfile", return_value=True):
            with patch("api.storage.views.file.remove"):
                admin_client.delete(f"/api/v2/files/{file_id}")

        assert not File.objects.filter(id=file_id).exists()

    def test_delete_file_not_allowed_removes_from_storage(self, admin_client):
    ...
    def test_delete_file_invalid_id_format(self, admin_client):
        """DELETE with invalid id format should return 404."""
        response = admin_client.delete("/api/v2/files/invalid")
        assert response.status_code == 404

    def test_delete_file_negative_id(self, admin_client):
        """DELETE with negative id should return 404."""
        response = admin_client.delete("/api/v2/files/-1")
        assert response.status_code == 404

    def test_delete_file_zero_id(self, admin_client):
        """DELETE with id=0 should return 404."""
        response = admin_client.delete("/api/v2/files/0")
        assert response.status_code == 404

    def test_delete_file_os_error_raises_api_exception(self, admin_client):
