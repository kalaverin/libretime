...
class TestFileViewSetDeleteNotFound:
    ...
    def test_delete_non_existent_large_id(self, admin_client):
        """DELETE with large non-existent ID should return 404."""
        response = admin_client.delete("/api/v2/files/999999999")
        assert response.status_code == 404

    def test_delete_zero_id(self, admin_client):
        """DELETE with ID=0 should return 404 (not valid PK)."""
        response = admin_client.delete("/api/v2/files/0")
        assert response.status_code == 404

    def test_delete_negative_id(self, admin_client):
        """DELETE with negative ID should return 404."""
        response = admin_client.delete("/api/v2/files/-1")
        assert response.status_code == 404

    def test_delete_negative_large_id(self, admin_client):
        """DELETE with large negative ID should return 404."""
        response = admin_client.delete("/api/v2/files/-999999")
        assert response.status_code == 404

    def test_delete_string_id(self, admin_client):
        """DELETE with string ID should return 404."""
        response = admin_client.delete("/api/v2/files/abc")
        assert response.status_code == 404

    def test_delete_mixed_string_id(self, admin_client):
        """DELETE with alphanumeric ID should return 404."""
        response = admin_client.delete("/api/v2/files/abc123")
        assert response.status_code == 404
    ...
    def test_delete_sql_injection_attempt(self, admin_client):
        ...
        response = admin_client.delete("/api/v2/files/1%20OR%201=1")
        assert response.status_code == 404

    def test_delete_sql_injection_union(self, admin_client):
    ...
    def test_delete_path_traversal_attempt(self, admin_client):
        """DELETE with path traversal should return 404."""
        response = admin_client.delete("/api/v2/files/../etc/passwd")
        assert response.status_code == 404

    def test_delete_special_chars_id(self, admin_client):
    ...
    def test_delete_unicode_id(self, admin_client):
        ...
        response = admin_client.delete("/api/v2/files/日本語")
        assert response.status_code == 404

    def test_delete_float_id(self, admin_client):
        """DELETE with float ID should return 404."""
        response = admin_client.delete("/api/v2/files/1.5")
        assert response.status_code == 404

    def test_delete_hex_id(self, admin_client):
        """DELETE with hex ID should return 404."""
        response = admin_client.delete("/api/v2/files/0x1A")
        assert response.status_code == 404

    def test_delete_empty_string_id(self, admin_client):
    ...
    def test_delete_double_delete(self, admin_client):
        ...
        with patch("api.storage.views.file.os.path.isfile", return_value=True):
            with patch("api.storage.views.file.remove"):
                response1 = admin_client.delete(f"/api/v2/files/{file_id}")
        assert response1.status_code == 204

        # Second delete should return 404, but due to T316 returns 204
        response2 = admin_client.delete(f"/api/v2/files/{file_id}")
        assert response2.status_code == 404

    def test_delete_after_manually_deleted_from_db(self, admin_client):
        ...
        response = admin_client.delete(f"/api/v2/files/{file_id}")
        assert response.status_code == 404

    def test_delete_deleted_file_still_on_disk(self, admin_client):
    ...
    def test_delete_max_int_id(self, admin_client):
        """DELETE with max integer ID should return 404."""
        # Test with PostgreSQL max int (2147483647)
        response = admin_client.delete("/api/v2/files/2147483647")
        assert response.status_code == 404

    def test_delete_max_int_plus_one_id(self, admin_client):
        """DELETE with max_int + 1 ID should return 404."""
        response = admin_client.delete("/api/v2/files/2147483648")
        assert response.status_code == 404

    def test_delete_scientific_notation_id(self, admin_client):
        """DELETE with scientific notation ID should return 404."""
        response = admin_client.delete("/api/v2/files/1e5")
        assert response.status_code == 404

    def test_delete_boolean_true_id(self, admin_client):
        """DELETE with 'true' as ID should return 404."""
        response = admin_client.delete("/api/v2/files/true")
        assert response.status_code == 404

    def test_delete_boolean_false_id(self, admin_client):
        """DELETE with 'false' as ID should return 404."""
        response = admin_client.delete("/api/v2/files/false")
        assert response.status_code == 404

    def test_delete_null_id(self, admin_client):
        """DELETE with 'null' as ID should return 404."""
        response = admin_client.delete("/api/v2/files/null")
        assert response.status_code == 404

    def test_delete_very_long_numeric_id(self, admin_client):
        """DELETE with very long numeric ID should return 404."""
        # 100 digit number
        long_id = "1" * 100
        response = admin_client.delete(f"/api/v2/files/{long_id}")
        assert response.status_code == 404

    def test_delete_url_encoded_id(self, admin_client):
        """DELETE with URL-encoded numeric ID should return 404."""
        # Use very large ID that won't exist
        response = admin_client.delete("/api/v2/files/9999999999999")
        assert response.status_code == 404
