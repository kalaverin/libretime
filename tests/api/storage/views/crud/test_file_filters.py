...
class TestFileViewSetFilters:
    ...
    def test_filter_by_md5_exact_match(self, api_client):
    ...
    def test_filter_by_md5_no_match(self, api_client):
        """Filter by non-existent md5 should return empty list."""
        baker.make(File, name="File", mime="audio/mpeg", md5="abc123")

        response = api_client.get("/api/v2/files?md5=nonexistent")
        data = response.json()

        assert response.status_code == 200
        assert data == []

    def test_filter_by_md5_partial_not_supported(self, api_client):
        """Partial md5 match should not work (exact match only)."""
        baker.make(File, name="File", mime="audio/mpeg", md5="abc123def456")

        response = api_client.get("/api/v2/files?md5=abc123")
        data = response.json()

        # Should not match partial
        assert len(data) == 0

    def test_filter_by_genre_exact_match(self, api_client):
        ...
        response = api_client.get("/api/v2/files?genre=Rock")
        data = response.json()
    ...
    def test_filter_by_genre_no_match(self, api_client):
        """Filter by non-existent genre should return empty list."""
        baker.make(File, name="File", mime="audio/mpeg", genre="Rock")

        response = api_client.get("/api/v2/files?genre=NonExistent")
        data = response.json()

        assert response.status_code == 200
        assert data == []

    def test_filter_by_genre_case_sensitive(self, api_client):
    ...
    def test_filter_by_genre_with_special_chars(self, api_client):
        ...
        response = api_client.get("/api/v2/files?genre=R%26B")  # URL encoded
        data = response.json()

        assert len(data) == 1
        assert data[0]["genre"] == "R&B"

    def test_filter_by_genre_null_excluded(self, api_client):
    ...
    def test_filter_by_genre_empty_string(self, api_client):
        ...
        response = api_client.get("/api/v2/files?genre=")
        data = response.json()
    ...
    def test_filter_combined_md5_and_genre(self, api_client):
        ...
        response = api_client.get("/api/v2/files?md5=abc123&genre=Rock")
        data = response.json()

        assert len(data) == 1
        assert data[0]["id"] == target.id

    def test_filter_combined_no_match(self, api_client):
        ...
        response = api_client.get("/api/v2/files?md5=abc123&genre=Jazz")
        data = response.json()

        assert len(data) == 0

    def test_filter_invalid_param_ignored(self, api_client):
        """Invalid filter parameters should be ignored."""
        file = baker.make(File, name="File", mime="audio/mpeg")

        response = api_client.get("/api/v2/files?invalid_param=value")
        data = response.json()

        # Should return all files, ignoring invalid filter
        assert len(data) >= 1

    def test_filter_with_unicode_genre(self, api_client):
        ...
        response = api_client.get("/api/v2/files?genre=日本語ジャズ")
        data = response.json()

        assert len(data) == 1
        assert data[0]["genre"] == "日本語ジャズ"

    def test_filter_by_md5_case_sensitive(self, api_client):
        """MD5 filter should be case-sensitive (hex strings)."""
        baker.make(File, name="File", mime="audio/mpeg", md5="abcdef123456")

        response = api_client.get("/api/v2/files?md5=ABCDEF123456")
        data = response.json()

        # Case-sensitive: uppercase != lowercase
        assert len(data) == 0

    def test_filter_without_params_returns_all(self, api_client):
