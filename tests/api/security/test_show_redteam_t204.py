...
class TestShowRetrieveIDEnumeration:
    ...
    def test_sequential_id_access(self, admin_client, admin_user):
        """Try to enumerate shows by sequential IDs."""
        admin_client.force_authenticate(user=admin_user)

        # Try to access sequential IDs
        for i in range(1, 10):
            response = admin_client.get(f"/api/v2/shows/{i}")
            # Should return 404 for non-existent, not reveal existence
            assert response.status_code in [200, 404]

    def test_negative_id(self, admin_client, admin_user):
    ...
    def test_zero_id(self, admin_client, admin_user):
    ...
    def test_very_large_id(self, admin_client, admin_user):
        """Try to retrieve with very large ID."""
        admin_client.force_authenticate(user=admin_user)
        response = admin_client.get("/api/v2/shows/999999999999999999")

        assert response.status_code in [404, 400]

    def test_float_id(self, admin_client, admin_user):
        """Try to retrieve with float ID."""
        admin_client.force_authenticate(user=admin_user)
        response = admin_client.get("/api/v2/shows/1.5")
...
class TestShowRetrieveSQLInjection:
    ...
    def test_sql_injection_in_id(self, admin_client, admin_user):
        ...
        for payload in sqli_payloads:
            response = admin_client.get(f"/api/v2/shows/{payload}")
            # Should not crash or return data
            if response.status_code == 500:
                pytest.fail(f"BUG: SQL injection causes crash: {payload}")
            assert response.status_code in [404, 400]

    def test_sql_injection_union(self, admin_client, admin_user):
    ...
    def test_sql_injection_error_message(self, admin_client, admin_user):
...
class TestShowRetrievePathTraversal:
    ...
    def test_path_traversal_dots(self, admin_client, admin_user):
        ...
        for path in traversal_paths:
            response = admin_client.get(f"/api/v2/shows/{path}")
            assert response.status_code in [404, 400]

    def test_null_byte_injection(self, admin_client, admin_user):
        """Try null byte injection."""
        admin_client.force_authenticate(user=admin_user)
        response = admin_client.get("/api/v2/shows/1%00../../../etc/passwd")
...
class TestShowRetrieveInformationDisclosure:
    ...
    def test_error_message_on_invalid_id(self, admin_client, admin_user):
        """Check error messages for information leakage."""
        admin_client.force_authenticate(user=admin_user)
        response = admin_client.get("/api/v2/shows/invalid")
    ...
    def test_stack_trace_disclosure(self, admin_client, admin_user):
        """Check if stack traces are exposed."""
        admin_client.force_authenticate(user=admin_user)
        response = admin_client.get("/api/v2/shows/☃")  # Unicode snowman
    ...
    def test_timing_disclosure(self, admin_client, admin_user):
        ...
        start = time.time()
        response1 = admin_client.get(f"/api/v2/shows/{show.id}")
        time_existing = time.time() - start

        # Request non-existing show
        start = time.time()
        response2 = admin_client.get("/api/v2/shows/99999")
...
class TestShowRetrieveMassAssignment:
    ...
    def test_get_with_body_params(self, admin_client, admin_user):
...
class TestShowRetrieveBusinessLogic:
    ...
    def test_retrieve_deleted_show(self, admin_client, admin_user):
        ...
        admin_client.force_authenticate(user=admin_user)
        response = admin_client.get(f"/api/v2/shows/{show_id}")

        # Should return 404
        assert response.status_code == 404

    def test_retrieve_with_special_chars_in_url(self, admin_client, admin_user):
