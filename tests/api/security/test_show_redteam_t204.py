...
class TestShowRetrieveIDEnumeration:
    ...
    def test_zero_id(self, guest_client, admin_user):
    ...
    def test_very_large_id(self, guest_client, admin_user):
        """Try to retrieve with very large ID."""
        guest_client.force_authenticate(user=admin_user)
        response = guest_client.get("/api/v2/shows/999999999999999999")

        assert response.status_code in [404, 400]

    def test_float_id(self, guest_client, admin_user):
        """Try to retrieve with float ID."""
        guest_client.force_authenticate(user=admin_user)
        response = guest_client.get("/api/v2/shows/1.5")
...
class TestShowRetrieveSQLInjection:
    ...
    def test_sql_injection_in_id(self, guest_client, admin_user):
        ...
        for payload in sqli_payloads:
            response = guest_client.get(f"/api/v2/shows/{payload}")
            # Should not crash or return data
            if response.status_code == 500:
                pytest.fail(f"BUG: SQL injection causes crash: {payload}")
            assert response.status_code in [404, 400]

    def test_sql_injection_union(self, guest_client, admin_user):
        """Try UNION-based SQL injection."""
        guest_client.force_authenticate(user=admin_user)
        response = guest_client.get(
            "/api/v2/shows/1%20UNION%20SELECT%20*%20FROM%20cc_subjs",
        )

        if response.status_code == 200:
            pytest.fail("BUG: UNION SQL injection returns data")

    def test_sql_injection_error_message(self, guest_client, admin_user):
...
class TestShowRetrievePathTraversal:
    ...
    def test_path_traversal_dots(self, guest_client, admin_user):
        ...
        for path in traversal_paths:
            response = guest_client.get(f"/api/v2/shows/{path}")
            assert response.status_code in [404, 400]

    def test_null_byte_injection(self, guest_client, admin_user):
        """Try null byte injection."""
        guest_client.force_authenticate(user=admin_user)
        response = guest_client.get("/api/v2/shows/1%00../../../etc/passwd")
...
class TestShowRetrieveInformationDisclosure:
    ...
    def test_error_message_on_invalid_id(self, guest_client, admin_user):
        """Check error messages for information leakage."""
        guest_client.force_authenticate(user=admin_user)
        response = guest_client.get("/api/v2/shows/invalid")
        ...
        if response.status_code == 404:
            ...
            for term in leaked_terms:
                if term.lower() in content.lower():
                    pytest.fail(
                        f"BUG: 404 leaks implementation details: {term}",
                    )

    def test_stack_trace_disclosure(self, guest_client, admin_user):
        """Check if stack traces are exposed."""
        guest_client.force_authenticate(user=admin_user)
        response = guest_client.get("/api/v2/shows/☃")  # Unicode snowman

        if response.status_code == 500:
            content = response.content.decode()
            if "traceback" in content.lower() or "stack" in content.lower():
                pytest.fail("BUG: Stack trace exposed in error")

    def test_timing_disclosure(self, guest_client, admin_user):
        ...
        start = time.time()
        response1 = guest_client.get(f"/api/v2/shows/{show.id}")
        time_existing = time.time() - start

        # Request non-existing show
        start = time.time()
        response2 = guest_client.get("/api/v2/shows/99999")
...
class TestShowRetrieveMassAssignment:
    ...
    def test_get_with_body_params(self, guest_client, admin_user):
...
class TestShowRetrieveBusinessLogic:
    ...
    def test_retrieve_deleted_show(self, guest_client, admin_user):
        ...
        guest_client.force_authenticate(user=admin_user)
        response = guest_client.get(f"/api/v2/shows/{show_id}")

        # Should return 404
        assert response.status_code == 404

    def test_retrieve_with_special_chars_in_url(self, guest_client, admin_user):
