...
class TestUserTokenLookup:
    ...
    def test_token_with_special_chars(self, session_client, admin_user):
        ...
        for token in special_tokens:
            response = session_client.get(f"/api/v2/user-tokens/{token}")
            # Should not crash
            assert response.status_code in [200, 404, 400]

    def test_very_long_token_lookup(self, session_client, admin_user):
        """Try to lookup with very long token."""
        session_client.force_authenticate(user=admin_user)

        long_token = "a" * 1000
        response = session_client.get(f"/api/v2/user-tokens/{long_token}")

        assert response.status_code in [200, 404, 400]

    def test_unicode_token_lookup(self, session_client, admin_user):
        """Try to lookup with unicode token."""
        session_client.force_authenticate(user=admin_user)

        unicode_token = "токен_пользователя_123"
        response = session_client.get(f"/api/v2/user-tokens/{unicode_token}")
...
class TestUserTokenBOLA:
    ...
    def test_access_other_user_token(
        ...
        session_client.force_authenticate(user=regular_user)
        response = session_client.get("/api/v2/user-tokens/admin_secret_token")
    ...
    def test_delete_other_user_token(
        ...
        session_client.force_authenticate(user=regular_user)
        response = session_client.delete("/api/v2/user-tokens/admin_secret_token")
...
class TestUserTokenMassAssignment:
    ...
    def test_update_token_value(self, session_client, admin_user):
...
class TestLoginAttemptLookup:
    ...
    def test_ip_with_malformed_format(self, session_client, admin_user):
        ...
        for ip in malformed_ips:
            response = session_client.get(f"/api/v2/login-attempts/{ip}")
            assert response.status_code in [200, 404, 400]

    def test_ipv6_lookup(self, session_client, admin_user):
        """Try to lookup with IPv6 address."""
        session_client.force_authenticate(user=admin_user)

        ipv6 = "2001:0db8:85a3:0000:0000:8a2e:0370:7334"
        response = session_client.get(f"/api/v2/login-attempts/{ipv6}")

        assert response.status_code in [200, 404, 400]

    def test_ip_with_path_traversal(self, session_client, admin_user):
...
class TestLoginAttemptBOLA:
    ...
    def test_access_other_user_login_attempt(
        ...
        session_client.force_authenticate(user=regular_user)
        response = session_client.get("/api/v2/login-attempts/192.168.1.100")
    ...
    def test_modify_login_attempt_count(self, session_client, admin_user):
    ...
    def test_delete_login_attempt_record(self, session_client, admin_user):
        ...
        session_client.force_authenticate(user=admin_user)
        response = session_client.delete("/api/v2/login-attempts/192.168.1.100")
...
class TestAuthBusinessLogic:
    ...
    def test_create_token_without_auth(self, session_client):
    ...
    def test_create_duplicate_token(self, session_client, admin_user):
