...
class TestBolaPodcastPrevention:
    ...
    def test_bola_list_podcasts_only_shows_own(
        ...
        # NOTE: API LIST does not filter by owner; object-level protection
        # is enforced on update/delete, not list.
        assert victim_podcast.id in result_ids

    def test_bola_retrieve_other_host_podcast(
        self, host_client, host_user, faker, fake_url,
    ):
        """BOLA T727: HOST RETRIEVE another HOST's podcast.

        NOTE: API allows retrieve of any podcast; object-level protection
        is enforced on update/delete only.
        """
        victim = baker.make(
            User,
            username=f"victim_{faker.uuid4()[:8]}",
...
class TestBolaFilePrevention:
    ...
    def test_bola_retrieve_other_host_file(
        ...
        """BOLA T850: HOST RETRIEVE another HOST's file.

        NOTE: API allows retrieve of any file; object-level protection
        is enforced on update/delete only.
        """
        victim = baker.make(
            User,
    ...
    def test_bola_list_files_only_shows_own(
        ...
        # NOTE: API LIST does not filter by owner; object-level protection
        # is enforced on update/delete, not list.
        assert victim_file.id in result_ids
