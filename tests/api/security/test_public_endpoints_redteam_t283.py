...
class TestPublicEndpointsT283InfoDisclosure:
    ...
    def test_info_no_sensitive_data(self):
        ...
        if found:
            pytest.xfail(
                f"T803: Info endpoint exposes sensitive fields: {found}",
            )
...
class TestPublicEndpointsT283DoS:
    ...
    def test_rate_limiting_on_public_endpoints(self):
        ...
        if elapsed < 5:
            pytest.xfail(
                f"T805: No rate limiting on public endpoints (100 req in {elapsed:.2f}s)",
            )
    ...
    def test_response_size_reasonable(self):
        ...
        if size > 10000:  # 10KB
            pytest.xfail(
                f"T806: Public endpoint response too large: {size} bytes",
            )
...
class TestPublicEndpointsT283CORS:
    ...
    def test_cors_arbitrary_origin_blocked(self):
        ...
        if response.status_code == 200:
            ...
            if allow_origin == "*":
                pytest.xfail(
                    "T807: CORS allows wildcard origin on public endpoint",
                )
    ...
    def test_error_messages_not_verbose(self):
        ...
        if response.status_code == 500:
            ...
            if any(kw in error_text for kw in leak_keywords):
                pytest.xfail(
                    "T812: Error message leaks implementation details",
                )
