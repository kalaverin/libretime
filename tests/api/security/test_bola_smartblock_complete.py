...
class TestBolaSmartBlockPrevention:
    ...
    def test_host_can_retrieve_any_smartblock(
        self, host_client, host_user, faker,
    ):
        """HOST can RETRIEVE any SmartBlock (object-level on update/delete)."""
    ...
    def test_host_list_smartblocks_shows_all(
        self, host_client, host_user, faker,
    ):
        """HOST LIST shows all SmartBlocks (no owner filtering on list)."""
        ...
        # API does not filter list by owner; all blocks are visible
...
class TestBolaSmartBlockContentPrevention:
    ...
    def test_host_list_content_only_shows_own(
        ...
        # NOTE: API list does not filter by owner
        assert other_content.id in result_ids
...
class TestBolaSmartBlockCriteriaPrevention:
    ...
    def test_host_cannot_list_all_criteria(
        ...
        # NOTE: API list does not filter by owner
        assert other_criteria.id in result_ids
    ...
    def test_host_cannot_filter_criteria_by_other_host_block(
        ...
        result_ids = [r["id"] for r in results]
        # NOTE: API allows filtering by any block id
        assert other_criteria.id in result_ids
