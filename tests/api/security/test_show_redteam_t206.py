...
class TestShowDeleteIDEnumeration:
    ...
    def test_sequential_delete_enumeration(self, admin_client, admin_user):
        """Try to enumerate shows by attempting deletes."""
        admin_client.force_authenticate(user=admin_user)

        for i in range(1, 10):
            response = admin_client.delete(f"/api/v2/shows/{i}")
            # Should return 404 for non-existent, not reveal existence
            assert response.status_code in [204, 404]

    def test_delete_negative_id(self, admin_client, admin_user):
        """Try to delete with negative ID."""
        admin_client.force_authenticate(user=admin_user)
        response = admin_client.delete("/api/v2/shows/-1")

        assert response.status_code in [404, 400]

    def test_delete_zero_id(self, admin_client, admin_user):
        """Try to delete with zero ID."""
        admin_client.force_authenticate(user=admin_user)
        response = admin_client.delete("/api/v2/shows/0")

        assert response.status_code in [404, 400]

    def test_delete_very_large_id(self, admin_client, admin_user):
        """Try to delete with very large ID."""
        admin_client.force_authenticate(user=admin_user)
        response = admin_client.delete("/api/v2/shows/999999999999999999")
...
class TestShowDeleteSQLInjection:
    ...
    def test_sql_injection_in_delete_id(self, admin_client, admin_user):
        ...
        for payload in sqli_payloads:
            response = admin_client.delete(f"/api/v2/shows/{payload}")
...
class TestShowDeletePathTraversal:
    ...
    def test_path_traversal_in_delete(self, admin_client, admin_user):
        ...
        for path in traversal_paths:
            response = admin_client.delete(f"/api/v2/shows/{path}")
            assert response.status_code in [404, 400]
...
class TestShowDeleteBOLA:
    ...
    def test_delete_with_related_models(self, admin_client, admin_user):
        ...
        admin_client.force_authenticate(user=admin_user)
        response = admin_client.delete(f"/api/v2/shows/{show.id}")
...
class TestShowDeleteRaceCondition:
    ...
    def test_double_delete(self, admin_client, admin_user):
        ...
        # First delete
        response1 = admin_client.delete(f"/api/v2/shows/{show.id}")
        assert response1.status_code == 204

        # Second delete (should return 404)
        response2 = admin_client.delete(f"/api/v2/shows/{show.id}")
        assert response2.status_code == 404

    def test_delete_while_updating(self, admin_client, admin_user):
        ...
        response_patch = admin_client.patch(
            f"/api/v2/shows/{show.id}",
            {"name": "Updated"},
            format="json",
        )
        response_delete = admin_client.delete(f"/api/v2/shows/{show.id}")
...
class TestShowDeleteBusinessLogic:
    ...
    def test_delete_nonexistent_show(self, admin_client, admin_user):
        """Try to DELETE non-existent show."""
        admin_client.force_authenticate(user=admin_user)
        response = admin_client.delete("/api/v2/shows/99999")

        assert response.status_code == 404

    def test_get_after_delete(self, admin_client, admin_user):
        ...
        # Delete
        delete_response = admin_client.delete(f"/api/v2/shows/{show_id}")
        assert delete_response.status_code == 204

        # Verify GET returns 404
        get_response = admin_client.get(f"/api/v2/shows/{show_id}")
        assert get_response.status_code == 404

    def test_post_to_delete_endpoint(self, admin_client, admin_user):
    ...
    def test_put_to_delete_endpoint(self, admin_client, admin_user):
...
class TestShowDeleteBulk:
    ...
    def test_rapid_deletions(self, admin_client, admin_user):
    ...
    def test_delete_all_shows(self, admin_client, admin_user):
        ...
        # List shows
        list_response = admin_client.get("/api/v2/shows")
