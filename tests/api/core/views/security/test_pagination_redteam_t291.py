...
class TestBOLAListEndpoints:
    ...
    def test_files_list_user_isolation(self, admin_client, faker):
        """User A should NOT see User B's files in LIST response. (T874)"""
        # Create users
        user_a = baker.make(
            User,
            username=f"user_a_{faker.user_name()}",
            role=Role.HOST,
        )
        user_b = baker.make(
            User,
            username=f"user_b_{faker.user_name()}",
            role=Role.HOST,
        )
        library = baker.make(
            Library,
    ...
    @pytest.mark.xfail(reason="BOLA vulnerability: T874", strict=False)
    def test_playlists_list_user_isolation(self, admin_client, faker):
        """User A should NOT see User B's playlists. (T874)"""
        user_a = baker.make(
            User,
            username=f"pl_a_{faker.user_name()}",
            role=Role.HOST,
        )
        user_b = baker.make(
            User,
            username=f"pl_b_{faker.user_name()}",
            role=Role.HOST,
        )
    ...
    def test_list_returns_only_owned_data(self, admin_client, faker):
        """Comprehensive BOLA test across multiple endpoints."""
        user_a = baker.make(
            User,
            username=f"owner_a_{faker.user_name()}",
            role=Role.HOST,
        )
        user_b = baker.make(
            User,
            username=f"owner_b_{faker.user_name()}",
            role=Role.HOST,
        )
        library = baker.make(
            Library,
...
class TestResourceExhaustionNoPagination:
    ...
    def test_list_large_dataset_response_time(self, admin_client, faker):
        ...
        user = baker.make(
            User,
            username=f"load_{faker.user_name()}",
            role=Role.HOST,
        )
        library = baker.make(
            Library,
    ...
    def test_concurrent_list_requests(self, admin_client, faker):
        """Multiple concurrent LIST requests = DoS vector."""
        user = baker.make(
            User,
            username=f"dos_{faker.user_name()}",
            role=Role.HOST,
        )
        library = baker.make(
            Library,
    ...
    def test_response_size_limits(self, admin_client, faker):
        """Response should have reasonable size limits."""
        user = baker.make(
            User,
            username=f"size_{faker.user_name()}",
            role=Role.HOST,
        )
        library = baker.make(
            Library,
...
class TestQueryParamFuzzing:
    ...
    def test_filter_sql_injection_files(self, admin_client, payload, faker):
        ...
        for param in filter_params:
            response = admin_client.get(f"/api/v2/files?{param}={payload}")
    ...
    def test_filter_path_traversal(self, admin_client, payload, faker):
        ...
        response = admin_client.get(f"/api/v2/files?name={payload}")
        # Should not crash
        assert response.status_code != 500

    @pytest.mark.parametrize("payload", NAUGHTY_STRINGS)
    def test_filter_naughty_strings(self, admin_client, payload, faker):
        """Edge case strings in filters."""
        user = baker.make(
            User,
            username=f"naughty_{faker.user_name()}",
            role=Role.HOST,
        )
        library = baker.make(
            Library,
    ...
    def test_pagination_params_rejected(self, admin_client, faker):
        """Pagination params should be handled (ignored or rejected)."""
        user = baker.make(
            User,
            username=f"page_{faker.user_name()}",
            role=Role.HOST,
        )
        library = baker.make(
            Library,
        ...
        for param in params:
            response = admin_client.get(f"/api/v2/files?{param}")
            # Should not crash
    ...
    def test_sort_param_sql_injection(self, admin_client, faker):
        """SQLi via sort/order parameters."""
        user = baker.make(
            User,
            username=f"sort_{faker.user_name()}",
            role=Role.HOST,
        )
        library = baker.make(
            Library,
        ...
        for sort_param in sqli_sorts:
            response = admin_client.get(f"/api/v2/files?{sort_param}")
            assert (
...
class TestMassDataExposure:
    ...
    def test_list_does_not_expose_sensitive_fields(self, admin_client, faker):
        """LIST should not expose internal/sensitive fields."""
        user = baker.make(
            User,
            username=f"expose_{faker.user_name()}",
            role=Role.HOST,
        )
        library = baker.make(
            Library,
    ...
    def test_list_vs_retrieve_field_consistency(self, admin_client, faker):
        """LIST should not expose more fields than RETRIEVE."""
        user = baker.make(
            User,
            username=f"consist_{faker.user_name()}",
            role=Role.HOST,
        )
        library = baker.make(
            Library,
    ...
    def test_list_field_count_reasonable(self, admin_client, faker):
        """LIST should return fewer fields than RETRIEVE (performance)."""
        user = baker.make(
            User,
            username=f"count_{faker.user_name()}",
            role=Role.HOST,
        )
        library = baker.make(
            Library,
...
class TestFilterAuthorizationBypass:
    ...
    def test_filter_by_other_user_id_blocked(self, admin_client, faker):
        """Filtering by other user's ID should not bypass auth. (T875)"""
        user_a = baker.make(
            User,
            username=f"filt_a_{faker.user_name()}",
            role=Role.HOST,
        )
        user_b = baker.make(
            User,
            username=f"filt_b_{faker.user_name()}",
            role=Role.HOST,
        )
        library = baker.make(
            Library,
    ...
    def test_filter_by_nonexistent_values(self, admin_client, faker):
        """Filtering by non-existent values should return empty, not error."""
        user = baker.make(
            User,
            username=f"nonex_{faker.user_name()}",
            role=Role.HOST,
        )
        library = baker.make(
            Library,
...
class TestUnicodeAndEncoding:
    ...
    def test_unicode_in_filter_values(self, admin_client, faker):
        """Unicode characters in filter values."""
        user = baker.make(
            User,
            username=f"unicode_{faker.user_name()}",
            role=Role.HOST,
        )
        library = baker.make(
            Library,
        ...
        for value in unicode_values:
            response = admin_client.get(f"/api/v2/files?name={value}")
            assert response.status_code in [
                200,
                400,
            ], f"Unicode '{value}' caused {response.status_code}"

    def test_filter_special_characters(self, admin_client, faker):
        """Special regex/wildcard characters in filters."""
        user = baker.make(
            User,
            username=f"special_{faker.user_name()}",
            role=Role.HOST,
        )
        library = baker.make(
            Library,
        ...
        for char in special_chars:
            response = admin_client.get(f"/api/v2/files?name={char}")
            assert (
                response.status_code != 500
...
class TestHttpMethodOverride:
    ...
    def test_method_override_on_list(self, admin_client, faker):
        """Method override headers should not bypass security."""
        user = baker.make(
            User,
            username=f"method_{faker.user_name()}",
            role=Role.HOST,
        )
        library = baker.make(
            Library,
        ...
        response = admin_client.get("/api/v2/files", **headers)
    ...
    def test_unsupported_methods_on_list(self, admin_client, faker):
        """Unsupported HTTP methods on LIST endpoints."""
        user = baker.make(
            User,
            username=f"unsupport_{faker.user_name()}",
            role=Role.HOST,
        )
        library = baker.make(
            Library,
        ...
        for method in methods:
            response = getattr(admin_client, method.lower())("/api/v2/files")
