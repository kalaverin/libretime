...
class TestLiveLogRedTeamBOLA:
    ...
    def test_bola_list_shows_all_live_logs(
        ...
        response = admin_client.get("/api/v2/live-logs")
    ...
    def test_bola_guest_user_can_access_logs(self, admin_client, guest_user):
        """
...
class TestLiveLogRedTeamBOPLA:
    ...
    def test_bopla_create_mass_assignment_id(
        ...
        response = admin_client.post("/api/v2/live-logs", data, format="json")
    ...
    def test_bopla_future_start_time(self, admin_client, admin_user, fake_word):
        """
...
class TestLiveLogRedTeamTimeBasedInjection:
    ...
    def test_sqli_in_start_time_filter(self, admin_client, admin_user):
        """
    ...
    def test_sqli_in_end_time_filter(self, admin_client, admin_user):
        """
        SQL Injection via end_time filter.
        """
        sqli_payload = "2026-04-09T10:00:00Z' OR '1'='1"

        response = admin_client.get(f"/api/v2/live-logs?end_time={sqli_payload}")

        if response.status_code == 500:
            pytest.xfail("T660: SQLi in end_time filter causes 500")

    def test_sqli_in_state_filter(self, admin_client, admin_user):
        """
        ...
        for state in sqli_states:
            response = admin_client.get(f"/api/v2/live-logs?state={state}")
...
class TestLiveLogRedTeamInjection:
    ...
    def test_xss_in_state_field(self, admin_client, admin_user):
        """
    ...
    def test_command_injection_in_state(self, admin_client, admin_user):
        """
...
class TestLiveLogRedTeamResourceConsumption:
    ...
    def test_rapid_live_log_creation(self, admin_client, admin_user, fake_word):
        """
    ...
    def test_bulk_live_log_list(self, admin_client, admin_user):
        """
    ...
    def test_rapid_updates(self, admin_client, admin_user, fake_word):
        """
...
class TestLiveLogRedTeamValidation:
    ...
    def test_create_empty_state(self, admin_client, admin_user):
        """
    ...
    def test_create_null_start_time(self, admin_client, admin_user, fake_word):
        """
    ...
    def test_create_very_long_state(self, admin_client, admin_user):
        """
    ...
    def test_create_invalid_state_values(self, admin_client, admin_user):
        """
...
class TestLiveLogRedTeamAuthentication:
    ...
    def test_unauthenticated_create(self, admin_client, fake_word):
    ...
    def test_unauthenticated_update(self, admin_client, fake_word):
    ...
    def test_unauthenticated_delete(self, admin_client, fake_word):
        """Unauthenticated DELETE should fail."""
        log = baker.make(LiveLog, state=fake_word, start_time=now())

        admin_client.logout()
        response = admin_client.delete(f"/api/v2/live-logs/{log.id}")
        assert response.status_code == 403

    def test_guest_user_create(self, admin_client, guest_user, fake_word):
...
class TestLiveLogRedTeamInformationDisclosure:
    ...
    def test_error_message_leaks_db_structure(self, admin_client, admin_user):
        ...
        """
        response = admin_client.get("/api/v2/live-logs?start_time=invalid')")
    ...
    def test_id_enumeration(self, admin_client, admin_user):
        """
        Different errors for existent vs non-existent IDs leak information.
        """
        response_existing = admin_client.get("/api/v2/live-logs/1")
        response_nonexistent = admin_client.get("/api/v2/live-logs/999999")
...
class TestLiveLogRedTeamHTTPMethodTampering:
    ...
    def test_trace_method_disabled(self, admin_client, admin_user, fake_word):
        """TRACE method should be disabled."""
        log = baker.make(LiveLog, state=fake_word, start_time=now())

        response = admin_client.trace(f"/api/v2/live-logs/{log.id}")
