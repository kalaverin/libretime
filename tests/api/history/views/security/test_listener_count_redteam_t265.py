...
class TestListenerCountRedTeamBOPLA:
    ...
    def test_bopla_create_mass_assignment_id(
        ...
        if response.status_code == 201:
            ...
            if result.get("id") == forced_id:
                pytest.xfail(
                    "T653: BOPLA - ListenerCount id mass assignment works",
                )
    ...
    def test_bopla_create_extra_fields_ignored(
        ...
        if response.status_code == 201:
            pytest.xfail(
                "T654: BOPLA - ListenerCount extra fields silently ignored",
            )
    ...
    def test_bopla_update_listener_count_manipulation(
        ...
        if response.status_code == 200:
            ...
            if result.get("listener_count") == 999999:
                ...
                pytest.xfail(
                    "T655: BOPLA - Can fake listener count statistics",
                )
    ...
    def test_sqli_in_timestamp_filter(self, admin_client, admin_user):
        ...
        for ts in sqli_timestamps:
            ...
            if response.status_code == 500:
                pytest.xfail("T657: SQLi in timestamp filter causes 500")
